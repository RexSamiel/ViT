...
def will_create_nan(value: float, bit: int) -> bool:
    ...
    corrupted = (
        torch.tensor(corrupted_int, dtype=torch.int32).view(torch.float32).item()
    )
    return torch.isnan(torch.tensor(corrupted)).item()


def find_nan_position(
    weight: torch.Tensor, bit: int, target_offset: int, embed_dim: int
    ...
    if len(flat) <= max_elements:
        ...
        if nan_count > 0:
            nan_indices = torch.nonzero(
                torch.isnan(t.flatten()), as_tuple=False
            ).squeeze()
            if nan_indices.numel() > 0:
                indices = (
                    nan_indices[:10].tolist()
                    if nan_indices.numel() > 10
                    else nan_indices.tolist()
                )
...
def run_test(
    ...
    if find_nan:
        ...
        if result is None:
            ...
            print(
                "The pretrained weights may not have values that create NaN with single bit flips."
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test fault injection in ViT attention"
    )
    ...
    parser.add_argument(
        "--row", type=int, default=10, help="Row in Q/K/V weight matrix"
    )
    parser.add_argument("--col", type=int, default=5, help="Column in weight matrix")
    parser.add_argument(
        "--print-all", action="store_true", help="Print all intermediate tensors"
    )
    parser.add_argument(
        "--find-nan",
        action="store_true",
