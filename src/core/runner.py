...
class MetricsTracker:
    ...
    def update_sdc(
        ...
        # Prediction changes (handle NaN predictions separately)
    ...
    def get_results(self) -> dict[str, float] | None:
        ...
        if len(self.sdc_magnitudes) > 0:
            ...
            results["msdc_avg"] = float("nan")  # or 0.0 if you prefer
        ...
        if len(self.sdc_1_levels) > 0:
            ...
            results["sdc_75pct"] = float("nan")
            results["sdc_100pct"] = float("nan")
            results["sdc_200pct"] = float("nan")
