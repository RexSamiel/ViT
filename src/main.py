...
def run(global_args, commands):
    ...
    ff_logits = model.ff_logits if (model.ff_logits.available and not save_args and not pa_args) else None
    ...
    for run_idx in range(global_args.repeat):
        ...
        for batch_idx, (images, labels) in enumerate(batches):
            ...
            if acc: acc.update_batch(outputs, labels)
            if sdc and ff_logits: sdc.update_batch(outputs, ff_logits.get_batch(batch_idx, len(images), device))
        ...
        if detector:
            ...
            for layer in injected_layers | weight_detected | input_detected:
                s = layer_stats.setdefault(layer, {"injected": 0, "detected": 0, "false_positive": 0, "input_faults": 0})
    ...
    if not save_args and not pa_args and all_runs:
        ...
        if acc:      acc.print_summary()
        if sdc:      sdc.print_summary()
        if detector and global_args.repeat == 1: detector.print_results()
    ...
    if save_args:
        ...
        if save_args.threshold or save_args.inputs or save_args.weights:
            ...
            if save_args.threshold:
                cs.calibrate_threshold(model, max_batches=max_batches, margin=save_args.margin)
            cs.save(include_weights=save_args.weights, save_calibration=save_args.threshold)
            cs.remove()
...
def _parse_commands(commands):
    ...
    for cmd_name, cmd_args in commands:
        if cmd_name == "fi":   fi_args   = cmd_args
        elif cmd_name == "hr": hr_args   = cmd_args
        elif cmd_name == "save": save_args = cmd_args
        elif cmd_name == "pa": pa_args   = cmd_args
...
def _print_timing(all_runs: list[dict], fi_args, global_args):
    ...
    if not (global_args.time or (fi_args is not None and getattr(fi_args, "time", False))):
        return
    all_batch_ms = [t for r in all_runs for t in r.get("times_ms", [])]
    per_run_mps  = [
        sum(r["times_ms"]) / (r.get("accuracy", {}).get("samples", 1) or 1)
        for r in all_runs if r.get("times_ms")
    ]
...
def _build_json_summary(all_runs: list[dict], global_args, fi_args, acc, sdc) -> dict:
    ...
    if per_run_mps:
        ...
        std_mps = (
            math.sqrt(sum((v - avg_mps) ** 2 for v in per_run_mps) / (len(per_run_mps) - 1))
            if len(per_run_mps) > 1 else 0.0
        )
    ...
    if sdc:
        ...
        std_logit_sdc = (
            math.sqrt(sum((v - avg_logit_sdc) ** 2 for v in sdc_vals) / (len(sdc_vals) - 1))
            if len(sdc_vals) > 1 else 0.0
        )
        summary.update({
    ...
    component = (
        "attention" if sub_component in ("qkv", "proj") else
        "mlp"       if sub_component in ("fc1", "fc2")  else None
    )
    summary["config"] = {
